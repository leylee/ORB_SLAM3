...
int main(int argc, char **argv) {
  ...
  if (argc < 5 || argc > 6) {
    cerr << endl
         << "Usage: rosrun ORB_SLAM3 Mono_Inertial path_to_vocabulary "
            "path_to_settings topic_image topic_imu [do_equalize]"
         << endl;
    ros::shutdown();
    return 1;
  }
  ...
  char *topic_image = argv[3];
  char *topic_imu = argv[4];
  ...
  ros::Subscriber sub_imu =
      n.subscribe(topic_imu, 1000, &ImuGrabber::GrabImu, &imugb);
  ros::Subscriber sub_img0 =
      n.subscribe(topic_image, 100, &ImageGrabber::GrabImage, &igb);
